    ...
    try {
      ...
      if (
        ...
        if (language.includes("-") || language.includes("_")) {
          fallbackPath =
            scriptPath + "localization/" + language.split(/[-_]/)[0] + ".json";
        }
      }
      ...
      if (!isDefaultLang(language)) {
        if (autoLang) {
          try {
            ...
            if (!languageJson.ok)
              throw new Error(`Missing language file: ${languageJson.status}`);
            ...
            if (fallbackPath) {
              ...
              if (!fallbackJson.ok)
                throw new Error(
                  `Missing language file: ${fallbackJson.status}`
                );
              langJson = {
                ...JSON.parse(await fallbackJson.text()),
                ...langJson,
              };
            }
          } catch (e) {
